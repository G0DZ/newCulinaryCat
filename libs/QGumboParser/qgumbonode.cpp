...
bool QGumboNode::isText() const
{
    return ptr_->type == GUMBO_NODE_TEXT;
}
...
QString QGumboNode::getText() const
{
    QString text;
    if (ptr_->type == GUMBO_NODE_TEXT)
        text = QString::fromUtf8(ptr_->v.text.text);
    return text;
}
